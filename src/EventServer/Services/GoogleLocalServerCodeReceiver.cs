...
public class GoogleLocalServerCodeReceiver : ICodeReceiver
{
  ...
  <body>
        ...
        CancellationToken ct)
    {
        ...
        return new AuthorizationCodeResponseUrl(coll.AllKeys.Where(k => k != null).ToDictionary(k => k!, k => coll[k]!));
    }
...
#error Unsupported target
    ...
    internal class CallbackUriChooser
    {
        ...
        internal CallbackUriChooser(IClock clock, TimeSpan timeout, Func<string, bool> listenerFailsFor)
        {
            ...
            _localhost = new UriStatistics(CallbackUriTemplateLocalhost, timeout, clock);
            _loopbackIp = new UriStatistics(CallbackUriTemplate127001, timeout, clock);
        }
